...
class TrafficTests(unittest.TestCase):
    def report(self):
        ...
        return {'format_version': 1, 'window_seconds': 1, 'observed_until_s': 2,
    ...
    def test_units_peaks_and_global_reconciliation(self):
        ...
        log = '2 Vote body message(s) were sent. 2 of them were received\nVote mini-protocol traffic sent: 4 message(s), 0.00 MB = 2 bodies (0.00 MB) + 1 announcement(s) (0.00 MB) + 1 request(s) (0.00 MB).'
        traffic.reconcile(rows, log)
        with self.assertRaises(ValueError):
            traffic.reconcile(rows, log.replace('2 bodies', '3 bodies'))
